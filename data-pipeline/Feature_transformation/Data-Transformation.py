...
class DataTransformation:
    ...
    def spark(self)->SparkSession:
        ...
        return SparkSession.builder.appName(app_name).config('spark.sql.execution.arrow.pyspark.enabled','true').config('spark.sql.adaptive.enabled','true').getOrCreate()
    ...
    def scale_features(self,df:DataFrame)->DataFrame:
        ...
        if numeric_config and numeric_config.get('method')=='standard':
            ...
            for c in scaling_cols:
                ...
                agg_exprs.append(F.stddev(c).alias(f'{c}_avg'))
            stats=df.groupBy().agg(*agg_exprs).collect()[0]
            for c in scaling_cols:
                mean_val=stats[f'{c}_avg']
                std_val=stats[f'{c}_val']
    ...
    def encode_categorical_features(self,df:DataFrame)-> DataFrame:
        ...
        category_config=self.config.get('categorical_encoding',{})
        if category_config.get('method')=='label':
            ...
            if valid_cols:
                logger.info(f'Applying StringInder to {len(valid_cols)} categorical columns..')
                out_cols=[f'{c}_idx' for c in valid_cols]
                indexer= StringIndexer(inputCols=valid_cols,outputCol=out_cols,handleInvalid='keep')
                model=indexer.fit(df)
                df=model.transform(df)
                for c in valid_cols:
                    df=df.withColumn(c,F.col(f'{c}_idx').cast('integer')).drop(f'{c}idx')
        return df
    ...
    def execute_pipeline(self)->None:
        ...
        df_transformed=df_cleaned.transform(self.extract_datetime_features).transform(self.extract_binary_features).transform(self.scale_features).transform(self.encode_categorical_features)
