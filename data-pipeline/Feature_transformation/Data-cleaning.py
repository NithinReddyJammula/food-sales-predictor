...
class DatasetCleaning:
    ...
    def clean_data(self,config_path:str):
        ...
        drop_columns=[col for col in config.get('drop_columns',[]) if col in df_raw.cols]
        ...
        if all(k in df.columns for k in group_keys) and original_time_col in df.columns:
            ...
            # 1. Create the hourly column with a distinct string name
            df = df.withColumn('hourly_time_index', F.date_trunc('hour', F.col(original_time_col)))

            # 2. Extract Promo values via Pivot (Grouping by the HOURLY index)
            ...
            agg_keys = group_keys + ['hourly_time_index']  # We group by the new hourly bucket!
            aggregations = []
            ...
            # 4. Join the feature flags back to the main aggregation
            df_final = main_agg_df.join(promo_pivot_df, on=agg_keys, how='left')

            # 5. Overwrite the original timestamp column with the perfect hourly data, and drop the temp column
            ...
            if 'avg_sell_price' in df.columns:
                df=df.filter((F.col('avg_selling_price')>=0 | F.col('avg_selling_price').isNull()))
            ...
            df.write.format('delta').mode('overwrite').option('overwriteSchema','true').option('delta.autooptimize.optimizeWrite','true').option("delta.autoOptimize.autoCompact", "true").saveAsTable(target_table)
